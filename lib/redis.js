...
const OPTIONS = [
    'Sabtu 26/9/2026, Jam 12.30 - 14.30, Smansa Balut',
    'Minggu 27/9/2026 Jam 12.30 - 14.30, Smansa Balut',
    'Terserah, Sabtu atau Minggu saya bisa, Smansa Balut'
];
...
export async function clearAllData() {
    ...
    if (client) {
        try {
            await Promise.all([
                client.del('gessit_survey_votes'),
                client.del('gessit_survey_respondents')
            ]);
            return true;
        } catch (e) {
            console.error('ioredis del error:', e);
        }
    }
    ...
    if (kvUrl && kvToken) {
        try {
            await fetch(kvUrl, {
                method: 'POST',
                headers: { Authorization: `Bearer ${kvToken}`, 'Content-Type': 'application/json' },
                body: JSON.stringify(["DEL", "gessit_survey_votes", "gessit_survey_respondents"])
            });
            return true;
        } catch (e) {
            console.error('REST del error:', e);
        }
    }
